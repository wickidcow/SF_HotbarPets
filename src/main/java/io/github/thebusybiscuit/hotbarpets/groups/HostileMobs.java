...
import io.github.thebusybiscuit.hotbarpets.pets.BreezePet;
...
public final class HostileMobs implements PetGroup {
    ...
    public void load(HotbarPets plugin) {
        ...
        if (plugin.isModernPetEnabled("breeze")) {
            new BreezePet(plugin.getItemGroup(), new SlimefunItemStack(
                "HOTBAR_PET_BREEZE", Material.BREEZE_SPAWN_EGG, "&bBreeze Pet", getName(),
                "&7Favourite Food: Wind Charges", "", "&fRight-Click: &7Launches a Wind Charge"
            ), new ItemStack(Material.WIND_CHARGE), new ItemStack[]{
                new ItemStack(Material.WIND_CHARGE), new ItemStack(Material.BREEZE_ROD), new ItemStack(Material.WIND_CHARGE),
                new ItemStack(Material.FEATHER), new ItemStack(Material.EMERALD), new ItemStack(Material.FEATHER),
                new ItemStack(Material.WIND_CHARGE), SlimefunItems.GOLD_18K, new ItemStack(Material.WIND_CHARGE)
            }).register(plugin);
        }
    }
}
